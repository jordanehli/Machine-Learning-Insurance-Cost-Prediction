...
Insurance <- read.csv("C:\\Users\\rath6\\Downloads\\insurance.csv")
...
n1 = round(0.8*n)
ii = sample(1:n,n)
train.set = Insurance[ii[1:n1],]
test.set = Insurance[ii[(n1+1):n],]

x_train = train.set[,c(1:6)]
y_train = train.set[,7]
x_test = test.set[,c(1:6)]
y_test = test.set[,7]

y_pred_base = mean(test.set$charges)

rmse_baseline = sqrt(mean((y_test - y_pred_base)^2))
rmse_baseline
...
#Feature Importance - Best Regression Model 

lm_step <- lm(charges ~.+bmi30*smoker, data = Insurance2)
step <- stepAIC(lm_step, direction = "both", trace = FALSE)
step

lm_best <- lm(formula = charges ~ sex + bmi + children + smoker + region + 
                bmi30 + age2 + smoker:bmi30, data=data.frame(charges= y_train, x_train))
lm_best_pred <- predict(lm_best, newdata = data.frame(x_test))
rmse_best <- sqrt(mean((lm_best_pred-y_test)^2))
print(paste("RMSE: ",rmse_best))

#Feature Importance      
lm_shapely <- calc.relimp(lm_best, type='lmg')
lm_shapely

lm_shapely$lmg

barplot(sort(lm_shapely$lmg,decreasing = TRUE),col=c(2:10),main="Relative Importance of Predictors",xlab="Predictor Labels",ylab="Shapley Value Regression",font.lab=2)
...
#### Refining it further with finding optimal no. of trees 

# Define the parameters for Random Forest with trees b/w 975 to 1025 

ntreev=seq(975, 1025, by = 5)
...
# Define the parameters for Random Forest with trees b/w 75 to 125 
ntreev=seq(75, 125, by = 5)
...
fit <- train(charges ~ .+smoker*age+bmi*age,
             ...
             ntree     = 120,
             data       = train.set)
importance_rf <-  varImp(fit, scale=FALSE)
rmse_rf = sqrt(mean((exp(y_test)-ypred_rf)^2))
...
#### Tree Pruning ####
#performs cross-validation to determine the optimal complexity parameter (k) for the tree model. It then fits an initial tree model, prunes it based on the optimal complexity parameter, and visualizes the pruned tree. Finally, it makes predictions on the test set using the pruned tree and calculates the root mean squared error (RMSE) as a measure of prediction accuracy
...
tree_initial =tree(charges ~. , data=train.set)
summary (tree_initial)
plot(tree_initial )
text(tree_initial, pretty =0)

cv_initial =cv.tree(tree_initial)
names(cv_initial)
plot(cv_initial$size ,cv_initial$dev ,type='b')
plot(cv_initial$k ,cv_initial$dev ,type='b')
#size is the number of subtrees
# k is the complexity parameter
cv_initial

# Subset the predictors without the response variable
predictors <- subset(train.set, select = -charges)
...
prune_tree <- prune.tree(tree_initial, best = 4)
plot(prune_tree)
text(prune_tree, pretty = 0)

# Convert non-numeric predictors in the test set to factors
test_predictors <- test.set %>%
  mutate(across(where(is.character), as.factor))

# Match factor levels in test set with training set
for (col in intersect(names(predictors), names(test_predictors))) {
  levels(test_predictors[[col]]) <- levels(predictors[[col]])
}

# Make predictions on the test set using the pruned tree
yhat <- predict(prune_tree, newdata = test_predictors)
prune_rmse <- sqrt(mean((yhat - test.set$charges)^2))
prune_rmse
